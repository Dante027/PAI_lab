...
function getExpiryStatus(expiry) {
  ...
  if (diffDays < 0) return { variant: 'danger', label: 'Przeterminowany' };
  if (diffDays <= 3) return { variant: 'warning', label: `Kończy się (${diffDays}d)` };
  return { variant: 'success', label: `OK (${diffDays}d)` };
}
